...
def mask_card(card: str) -> str:
    """Return a UI-safe card preview without exposing PAN or CVV."""
...
def _build_charged_dm(
    ...
    return (
        f'<b><a href="{CHANNEL_LINK}">[❆]</a> CHARGED HIT '
        f'<tg-emoji emoji-id="{charged_eid}">💎</tg-emoji></b>\n'
        f'<b>╭────────────────────</b>\n'
        f'<b>│ <tg-emoji emoji-id="{CARD_EMOJI_ID}">💳</tg-emoji> Card  <code>{mask_card(cc_formatted)}</code></b>\n'
        f'<b>│ Gate   {safe_price} {safe_cur}</b>\n'
        f'<b>├────────────────────</b>\n'
        f'<b>│ Result <code>{safe_resp}</code></b>\n'
        f'<b>│ BIN    <code>{bin_info_str}</code></b>\n'
        f'<b>├────────────────────</b>\n'
        f'<b>│ <tg-emoji emoji-id="{TIME_EMOJI_ID}">⏱</tg-emoji> Time   {time_str}</b>\n'
        f'<b>│ <tg-emoji emoji-id="{USER_EMOJI_ID}">👤</tg-emoji> User   {user_link} '
...
def _build_progress_text(session: dict) -> str:
    """
    Progress message UI:
      A compact, scan-friendly progress card for mass checks.
    """
    ...
    total = session["total"]
    status = session.get("status", "CHECKING")
    ...
    return (
        ...
        f'<b>┃ <tg-emoji emoji-id="{PROG_PROGRESS_EMOJI_ID}">🔄</tg-emoji> {status_label}  ·  {checked}/{total}</b>\n'
        f'<b>┣━━━━━━━━━━━━━━━━━━━━━━┫</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_CHARGED_EMOJI_ID}">💎</tg-emoji> Charged   {session["charged"]}</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_LIVE_EMOJI_ID}">✅</tg-emoji> Live      {session["approved"]}</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_DEAD_EMOJI_ID}">❌</tg-emoji> Declined  {session["dead"]}</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_ERRORS_EMOJI_ID}">⚠️</tg-emoji> Errors    {session["errors"]}</b>\n'
        f'<b>┣━━━━━━━━━━━━━━━━━━━━━━┫</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{TIME_EMOJI_ID}">⏱</tg-emoji> {time_str}   ·   {user_link} '
        f'<tg-emoji emoji-id="{plan_emoji_id}">⭐</tg-emoji></b>\n'
        f'<b>╰━━ <tg-emoji emoji-id="{DEV_EMOJI_ID}">⚡</tg-emoji> {dev_link} ━━╯</b>'
    )
...
async def handle_result_callback(callback: types.CallbackQuery, callback_data: MshResultCallback):
    try:
        ...
        caption = (
            f"<b>╭━━ REPORT READY ━━╮</b>\n"
            f"<b>┃ {type_emojis.get(result_type, '📁')} {type_labels.get(result_type, 'All').upper()} RESULTS</b>\n"
            f"<b>┃ Total   {total_count}</b>\n"
...
async def msh_command(message: types.Message):
    if not is_gate_enabled("msh"):
        await message.reply(
            "🛠 <b>Mass audit is temporarily offline.</b>\n"
    ...
    if not proxies:
        await message.reply(
            "⚠️ <b>Proxy pool unavailable</b>\n\n"
            "Add one proxy per line to <code>proxies.txt</code> or <code>px.txt</code>.",
            parse_mode="HTML"
        )
        return
...
async def process_mass_check_background(
    ...
    initial_text = (
        f'<b>╭━━━ <tg-emoji emoji-id="{PROG_GATE_EMOJI_ID}">🛒</tg-emoji> SHOPIFY MASS AUDIT ━━━╮</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_PROGRESS_EMOJI_ID}">🔄</tg-emoji> STARTING  ·  0/{total_cards}</b>\n'
        f'<b>┣━━━━━━━━━━━━━━━━━━━━━━┫</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_CHARGED_EMOJI_ID}">💎</tg-emoji> Charged   0</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_LIVE_EMOJI_ID}">✅</tg-emoji> Live      0</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_DEAD_EMOJI_ID}">❌</tg-emoji> Declined  0</b>\n'
        f'<b>┃ <tg-emoji emoji-id="{PROG_ERRORS_EMOJI_ID}">⚠️</tg-emoji> Errors    0</b>\n'
        f'<b>┣━━━━━━━━━━━━━━━━━━━━━━┫</b>\n'
        f'<b>┃ ⏱ 0s   ·   {user_link} <tg-emoji emoji-id="{plan_emoji_id}">⭐</tg-emoji></b>\n'
        f'<b>╰━━ <tg-emoji emoji-id="{DEV_EMOJI_ID}">⚡</tg-emoji> {dev_link} ━━╯</b>'
    )
